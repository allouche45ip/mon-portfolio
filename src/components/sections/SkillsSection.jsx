...
const SkillsSection = () => {
  ...
  const skills = [
    ...
    { name: "Node.js", level: 60, color: "from-green-400 to-emerald-500", image: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg", description: "Backend Runtime" },
    { name: "Django", level: 60, color: "from-purple-400 to-pink-500", image: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/django/django-plain.svg", description: "Python Framework" },
    { name: "Tailwind CSS", level: 95, color: "from-pink-400 to-rose-500", image: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tailwindcss/tailwindcss-plain.svg", description: "Utility‑First CSS Framework", link: "https://tailwindcss.com" },
  ]
  ...
  const CircularProgress = ({ skill, index, isVisible }) => {
    ...
    useEffect(() => {
      if (isVisible) {
        const timer = setTimeout(() => setProgress(skill.level), index * 200)
  ...
  return (
    ...
    >
      <div className="absolute inset-0 opacity-10">
        <div className="absolute top-20 left-10 w-72 h-72 bg-cyan-400 rounded-full mix-blend-multiply blur-xl animate-blob"></div>
        <div className="absolute top-40 right-10 w-72 h-72 bg-purple-400 rounded-full mix-blend-multiply blur-xl animate-blob animation-delay-2000"></div>
        <div className="absolute bottom-20 left-20 w-72 h-72 bg-pink-400 rounded-full mix-blend-multiply blur-xl animate-blob animation-delay-4000"></div>
      </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {skills.map((skill, idx) => (
            <div
              key={idx}
              className={`bg-gray-900/50 backdrop-blur-sm p-8 rounded-2xl border border-gray-700 hover:border-cyan-400/50 transition-all duration-500 transform hover:scale-105 hover:-translate-y-2 group relative ${isVisible ? "animate-fade-in-up" : "opacity-0"}`}
              style={{ animationDelay: `${idx * 100}ms` }}
            >
              <CircularProgress skill={skill} index={idx} isVisible={isVisible} />
              ...
              <div className="w-full bg-gray-700 rounded-full h-2 mb-4">
                <div
                  className={`h-2 rounded-full transition-all duration-2000 ease-out bg-gradient-to-r ${skill.color}`}
                  style={{ width: isVisible ? `${skill.level}%` : "0%", transitionDelay: `${idx * 200}ms` }}
                ></div>
              </div>
