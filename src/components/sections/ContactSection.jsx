...
// 🔁 Real backend now — set VITE_API_URL in your .env for production,
// falls back to local Django dev server during development.
const API_URL = import.meta.env.VITE_API_URL || "http://127.0.0.1:8000/api/contact/messages/"
...
export default function ContactSection() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 🔁 No more "no-cors" blind-fire — this is a real request, and we
      // actually read the response, so failures are no longer silent.
      ...
      if (!response.ok) {
        // DRF returns field errors as JSON, e.g. { "email": ["This field is required."] }
        const data = await response.json().catch(() => null)
        const firstError =
          data && typeof data === "object"
            ? Object.values(data).flat()[0]
            : null
        throw new Error(firstError || "Le serveur a refusé la requête.")
      }
      ...
      console.error("Fetch error:", err)
      // Network failure (server down, CORS, no internet) vs. server-rejected request
